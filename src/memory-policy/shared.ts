 ...
 * fact, and the nudge's cost (a visible "Stop hook feedback" block plus a
 * full-context rewake turn) is pure noise. Mirrored in
...
export function renderClaudeCodeStopNudgePrompt(): string {
  // ONE line on purpose: Claude Code renders Stop-hook additionalContext
  // verbatim as a visible "Stop hook feedback" block (suppressOutput only
  // hides raw stdout — verified empirically on 2.1.175), so every character
  // here is user-facing. Trigger detail and dials live in the memory skill
  // and host rules; this is just the last-chance check plus a terse close for
  // the rewake turn the injection forces.
  return (
    'No memories were stored this session. If a durable preference, correction, decision, pattern, ' +
    'or insight emerged, store it now via the store_memory tool (recall related -> store -> verify -> associate; bare tags). ' +
    'Do NOT store session summaries, progress notes, or confirmations - if nothing qualifies, reply with exactly: Nothing durable to store.'
  );
}
