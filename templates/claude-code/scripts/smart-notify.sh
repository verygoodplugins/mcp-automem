...
if [ -f "$TRANSCRIPT_PATH" ] && [ "$HOOK_EVENT" = "Stop" ]; then
    ...
    LAST_ASSISTANT_MSG=$(tail -10 "$TRANSCRIPT_PATH" | \
                        jq -r 'select(.message.role == "assistant") | (.message.content[0].text // empty)' | \
                        tail -1 | \
                        tr '\n' ' ' | \
                        cut -c1-60)
else
...
if command -v terminal-notifier >/dev/null 2>&1; then
    ...
    # Use argv to avoid AppleScript syntax errors when variables contain quotes/parentheses/etc.
    osascript - "$MSG" "Claude Code ($SESSION_DIR) - $TITLE" "$SOUND" <<'APPLESCRIPT'
on run argv
  set msg to item 1 of argv
  set theTitle to item 2 of argv
  set soundName to item 3 of argv
  display notification msg with title theTitle sound name soundName
end run
APPLESCRIPT
fi
