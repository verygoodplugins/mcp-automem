...
try {
    ...
    $deployPattern = '(^|\s)(deploy|kubectl apply|helm (install|upgrade)|docker push|az webapp deploy|aws (s3 sync|ecs|lambda)|gcloud (app deploy|run deploy)|terraform apply|pulumi up|cdk deploy|fly deploy|railway up|vercel|netlify deploy|gh-pages)'
