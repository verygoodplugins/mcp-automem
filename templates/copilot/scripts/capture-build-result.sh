...
LOG_FILE="$HOME/.copilot/logs/build-results.log"
...
if [ -z "$COMMAND" ] || ! echo "$COMMAND" | grep -qiE "(^|\\s)(npm (run )?build|yarn build|pnpm build|vite build|webpack|rollup|parcel|go build|cargo build|dotnet build|msbuild|bicep build|az bicep build|gradle|mvn|make|composer)"; then
    exit 0
fi
...
# cargo must be checked before go — "cargo build" contains "go build" as a substring.
    ...
    BUILD_TOOL="dotnet"
elif echo "$COMMAND" | grep -q "bicep build\|az bicep build"; then
    BUILD_TOOL="bicep"
...
TOOL_TO_LANG = {
    ...
    "dotnet": "csharp", "bicep": "bicep", "make": "c", "composer": "php",
}
