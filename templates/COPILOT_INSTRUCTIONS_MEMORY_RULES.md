...
- `limit 10->30` and `time_query "last 30 days"->"last 90 days"` compound: 2.5x useful results with zero score-quality loss.
...
- `store_memory` - save content with tags, importance (0.0-1.0), type, metadata, optional `t_valid` / `t_invalid`. Supports **batch mode** via `memories: [...]` (up to 500 items, no per-item `id`/`embedding`/`t_valid`/`t_invalid`).
