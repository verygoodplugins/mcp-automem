...
/**
 * pwsh cold-starts the .NET runtime on its first spawn, and on a CI runner that alone
 * can exceed vitest's 5s default — this suite's first test flaked twice on unrelated
 * PRs. bash needs no such allowance, so the budget is per shell rather than global.
 */
const SHELL_TIMEOUT_MS: Record<Shell, number> = { bash: 5_000, pwsh: 30_000 };

function sessionStartSuite(shell: Shell) {
  const timeoutMs = SHELL_TIMEOUT_MS[shell];
  ...
  it(
    'exits cleanly and outputs valid JSON with additionalContext',
    () => {
  ...
  it(
    'emits the Copilot CLI top-level additionalContext envelope by default',
    () => {
      const result = runSessionStart(shell);
      const parsed = JSON.parse(result.stdout);
  ...
  it(
    'emits the VS Code hookSpecificOutput envelope when requested',
    () => {
      const result = runSessionStart(shell, {
  ...
  it(
    'injects a Phase 1 preference recall using the bare "preference" tag',
    () => {
  ...
  it(
    'injects a Phase 2 task-context recall with project-slug gate + 90-day window',
    () => {
  ...
  it(
    'injects a Phase 3 on-demand debugging recall without a tag gate',
    () => {
      const { additionalContext: ctx } = runSessionStart(shell);
  ...
  it(
    'tool-call bodies use ONLY bare tags (no namespace prefixes)',
    () => {
      const { additionalContext: ctx } = runSessionStart(shell);
