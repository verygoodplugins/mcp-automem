...
const SCENARIOS = [
  {
    ...
    assert: async (ctx) => {
      ...
      r.push(A('codex hooks.json written', ctx.homeNew.includes('.codex/hooks.json')));
      r.push(A('session-start hook written',
        ctx.homeNew.includes('.codex/hooks/automem-session-start.sh')));
      r.push(A('drain-queue script written', ctx.homeNew.includes('.codex/scripts/drain-queue.sh')));
...

  {
    ...
    description: 'Sibling client: headless existing-target install for Claude Code (proves the symmetric writer exists).',
    mock: { mode: 'healthy', expectToken: TOKEN },
    steps: (ctx) => [
      {
        kind: 'direct',
...

  {
    ...
    assert: async (ctx) => {
      ...
      // hooks.json must still be valid JSON after the merge-on-reinstall.
      let valid = false;
      try {
        JSON.parse(await readFile(path.join(ctx.home, '.codex', 'hooks.json'), 'utf8'));
        valid = true;
      } catch (e) {
        valid = false;
      }
      r.push(A('hooks.json valid JSON after reinstall', valid));
      const env = await readEnvFile(ctx.cwd);
