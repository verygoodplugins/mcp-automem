...
config();
// Optional: allow upstream supervisors to set a stable process title for safe cleanup.
  ...
  try {
    switch (name) {
      case "store_memory": {
        ...
        const originalLength = result.original_length;
        const summarizedLength = result.summarized_length;
        if (summarized) {
